...
const Ico = {
  ...
  wave: (p) => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M2 12s2-3 5-3 5 3 5 3 2 3 5 3 5-3 5-3"/><path d="M2 18s2-3 5-3 5 3 5 3 2 3 5 3 5-3 5-3"/></svg>),
  share: (p) => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M18 8a3 3 0 1 0-2.83-4M6 12a3 3 0 1 0 0 0zm12 4a3 3 0 1 0-2.83 4"/><path d="M8.6 13.5l6.8 4M15.4 6.5l-6.8 4"/></svg>),
};
...
function computeStatus(now = new Date()) {
  ...
  const today = D.horarioSala[day];
  for (const [a, b] of today.rangos) {
    if (mins >= a && mins < b) {
      return { open: true, text: `Abierto ahora · cierra a las ${fmt(b)}` };
    }
  }
  // próxima apertura hoy
  for (const [a] of today.rangos) {
    if (mins < a) return { open: false, text: `Cerrado · abre hoy a las ${fmt(a)}` };
  }
  // siguientes días
  for (let i = 1; i <= 7; i++) {
    const d = (day + i) % 7;
    ...
    if (info.rangos.length) {
      ...
      return { open: false, text: `Cerrado · abre ${when} a las ${fmt(info.rangos[0][0])}` };
    }
  }
...
const waLink = (text) => `https://wa.me/${D.negocio.whatsapp}?text=${encodeURIComponent(text)}`;
  ...
  return (
    <nav className={"nav" + (scrolled ? " scrolled" : "")}>
      ...
      <div className="nav-links">
        ...
        <a href="#musica">Música en directo</a>
        <a href="#galeria">Galería</a>
        <a href="#ubicacion">Dónde estamos</a>
      </div>
      <button className="btn btn-coral nav-cta" onClick={onReserve}>Reservar mesa</button>
    </nav>
  );
}
  ...
  return (
    <header className="hero" id="top">
      <div className="hero-bg">
        <img src="assets/terraza-atardecer.png" alt="Terraza de El Fantástico al atardecer sobre el puerto de Laredo" />
      </div>
      <div className="hero-inner">
        <div className="wrap">
          <h1>El<br/><span className="line2">Fantástico</span></h1>
          ...
          <div className="hero-status">
            <span className={"status-pill " + (status.open ? "open" : "closed")}>
              <span className="led"></span>{status.text}
            </span>
            <span className="hero-rating">
              <span className="stars">★★★★☆</span>
              <span>{D.negocio.rating}</span>
              <span className="muted">· {D.negocio.reseñas} reseñas</span>
            </span>
          </div>
        </div>
      </div>
      <div className="scroll-cue"><span className="mouse"></span>Desliza</div>
    </header>
  );
}
...
function Experiencia() {
  ...
  return (
    <section className="sec-light sec-pad" id="experiencia">
      <div className="wrap">
        <div className="sec-head reveal">
          <span className="eyebrow">El alma del sitio</span>
          <h2>No es un restaurante de pescado serio y aburrido.</h2>
          <p>{D.negocio.claim}</p>
        </div>
        ...
        <div className="exp-feature reveal">
          ...
          <div className="txt">
            <span className="eyebrow amber" style={{ color: "var(--amber)" }}>Atardecer en la dársena</span>
            <h3>La mejor mesa para ver caer el sol.</h3>
            <p>Terraza al borde del agua, comedor interior con plantas y luz cálida. Vengas a picar o a comer completo, aquí se come con vistas y se brinda con lo mejor del Cantábrico.</p>
          </div>
        </div>
      </div>
    </section>
...
function Carta() {
  ...
  return (
    <section className="sec-sand sec-pad" id="carta">
      <div className="wrap">
        <div className="sec-head reveal">
          <span className="eyebrow">Para compartir</span>
          <h2>Producto, pescado y manos al aire.</h2>
        ...
        <div className="carta-tabs reveal">
          {D.cartaTabs.map((t) => (
            <button key={t.id} className={"carta-tab" + (t.id === tab ? " active" : "")} onClick={() => setTab(t.id)}>{t.label}</button>
          ))}
        </div>
            ...
            <div className="dish" key={i}>
              ...
              <div className="dish-body">
                <div className="dish-top">
                  ...
                  <span className="dish-price">{p.precio}</span>
                </div>
...
function Musica() {
  return (
    <section className="sec-navy sec-pad musica" id="musica">
      <div className="wrap">
        <div className="sec-head reveal">
          <span className="eyebrow">Lo que nos hace diferentes</span>
          <h2>Música en directo<br/>en el puerto.</h2>
...
function Galeria({ onOpen }) {
  const classes = ["big", "tall", "", "", "tall"];
  return (
    <section className="sec-light sec-pad" id="galeria">
      <div className="wrap">
        <div className="sec-head reveal">
          <span className="eyebrow">Galería</span>
          <h2>El puerto, la terraza y los platos.</h2>
        </div>
        <div className="gal-grid reveal">
          {D.galeria.map((g, i) => (
            <div className={"gal-item " + (classes[i] || "")} key={i} onClick={() => onOpen(i)}>
...
function Lightbox({ index, setIndex }) {
  ...
  useEffect(() => {
    if (!open) return;
    const fn = (e) => { if (e.key === "Escape") setIndex(null); if (e.key === "ArrowRight") go(1); if (e.key === "ArrowLeft") go(-1); };
...
function Reseñas() {
  return (
    <section className="sec-sand sec-pad" id="resenas">
      <div className="wrap">
        <div className="sec-head reveal">
          <span className="eyebrow">Lo que dicen</span>
          <h2>1.486 motivos para venir.</h2>
        </div>
        <div className="rev-top reveal">
          <div className="rev-score">
            <div className="big">{D.negocio.rating}</div>
            <div>
              <div className="stars">★★★★☆</div>
              <div className="cnt">{D.negocio.reseñas} opiniones</div>
            </div>
          </div>
          <span className="rev-google">Valoración media en Google</span>
        </div>
        <div className="rev-grid">
          {D.reseñas.map((r, i) => (
            <div className="rev-card reveal" key={i} style={{ transitionDelay: `${i * 80}ms` }}>
              <div className="stars">{stars(r.estrellas)}</div>
              <p>“{r.texto}”</p>
        ...
        <div className="loc-grid">
          ...
          <div className="info-card reveal">
            <div className="info-row">
              ...
              <div><div className="lbl">Dirección</div>
                <a className="val" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(D.negocio.mapsQuery)}`} target="_blank" rel="noopener">{D.negocio.direccion}</a>
              </div>
            </div>
            <div className="info-row">
              ...
              <div style={{ flex: 1 }}>
                ...
                <span className={"status-pill " + (status.open ? "open" : "closed")} style={{ marginTop: 6, marginBottom: 4 }}>
                  <span className="led"></span>{status.text}
                </span>
                      ...
                      return (
                        <tr key={d} className={d === todayIdx ? "today" : ""}>
                          ...
                          {cerrado
                            ...
                            : <td>{info.rangos.map(([a,b]) => `${fmt(a)}–${fmt(b)}`).join(" · ")}</td>}
                        </tr>
                      );
                    })}
                  </tbody>
            ...
            <a className="btn btn-dark" href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(D.negocio.mapsQuery)}`} target="_blank" rel="noopener" style={{ width: "100%" }}>{Ico.pin({})} Cómo llegar</a>
          </div>
        </div>
      </div>
    </section>
...
function CtaFinal({ onReserve }) {
  return (
    <section className="sec-navy sec-pad cta-final" id="reservar">
      <div className="wrap">
        <span className="eyebrow reveal" style={{ color: "var(--amber)" }}>Te esperamos</span>
        <h2 className="reveal">Reserva tu mesa<br/>en <em>El Fantástico</em>.</h2>
        <p className="reveal">Mejor con tiempo en verano y los días de música en directo.</p>
        <div className="hero-ctas reveal">
          <button className="btn btn-coral" onClick={onReserve}>{Ico.cal()} Reservar mesa</button>
          <a className="btn btn-wa" href={waLink("¡Hola! Me gustaría reservar mesa en El Fantástico.")} target="_blank" rel="noopener">{Ico.wa()} Escribir por WhatsApp</a>
          <a className="btn btn-ghost" href={`tel:${D.negocio.telefono.replace(/\s/g, "")}`}>{Ico.phone()} {D.negocio.telefono}</a>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <footer>
      <div className="wrap">
        ...
        <p style={{ marginTop: 8 }}>{D.negocio.direccion} · {D.negocio.telefono}</p>
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="field-row">
          <div className="field"><label>Personas</label>
            <select value={f.personas} onChange={set("personas")}>
          ...
          <div className="field"><label>Hora</label>
            <select value={f.hora} onChange={set("hora")}>
              {["13:00","13:30","14:00","14:30","20:00","20:30","21:00","21:30","22:00","22:30"].map((h) => <option key={h} value={h}>{h}</option>)}
            </select>
          </div>
        </div>
        <div className="field-row">
